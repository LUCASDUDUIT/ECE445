...
static void RH_setModeIdle(void)
{
    if (RHmode != RHModeIdle) {
    	...
    	HAL_GPIO_WritePin(GPIOA, TRANSMIT_Pin, GPIO_PIN_RESET);
    	RHmode = RHModeIdle;
    }
}
...
static void RH_setModeTx(void)
{
    if (RHmode != RHModeTx) {
    	...
    	HAL_GPIO_WritePin(GPIOA, TRANSMIT_Pin, GPIO_PIN_SET);
    	RHmode = RHModeTx;
    }
}
...
{
    Bool_E value;
    value = HAL_GPIO_ReadPin(GPIOA, RECEIVE_Pin);
    return value;
}
...
{
	HAL_GPIO_WritePin(GPIOA, TRANSMIT_Pin, value);
}
